@extends('adminlte::page')

@section('title', 'Juegos Activos')

@section('content_header')
    <h1>Juegos Activos</h1>
@stop

@section('content')
    <div class="card">
        <div class="card-header">
            <a href="{{ route('admin.games.create') }}" class="btn btn-primary">Añadir Nuevo Juego</a>
            <a href="{{ route('admin.games.report') }}" class="btn btn-secondary">Generar Informe</a>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th style="width: 10px">Id</th>
                            <th>Nombre</th>
                            <th class="d-none d-sm-table-cell">Descripción</th>
                            <th class="d-none d-md-table-cell">Categoría</th>
                            <th class="d-none d-md-table-cell">Recomendado</th>
                            <th class="d-none d-lg-table-cell">Visitas</th>
                            <th class="d-none d-lg-table-cell">Precio</th>
                            <th style="width: 120px">Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($games as $game)
                            <tr>
                                <td>{{ $game->slug }}</td>
                                <td>{{ $game->name }}</td>
                                <td class="d-none d-sm-table-cell">{{ Str::limit($game->description, 50) }}</td>
                                <td class="d-none d-md-table-cell">{{ $game->category }}</td>
                                <td class="d-none d-md-table-cell">{{ $game->recommended ? 'Sí' : 'No' }}</td>
                                <td class="d-none d-lg-table-cell">{{ $game->visits }}</td>
                                <td class="d-none d-lg-table-cell">{{ $game->price }}</td>
                                <td>
                                    <div class="btn-group">
                                        <button type="button" class="btn btn-secondary dropdown-toggle dropdown-icon" data-toggle="dropdown">
                                            <span class="sr-only">Toggle Dropdown</span>
                                        </button>
                                        <div class="dropdown-menu" role="menu">
                                            <a href="{{ route('admin.games.edit', $game->slug) }}" class="dropdown-item">
                                                <i class="fas fa-pencil-alt mr-2"></i>
                                            </a>
                                            <div class="dropdown-divider"></div>
                                            <form action="{{ route('admin.games.destroy', $game->slug) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="dropdown-item text-danger" onclick="return confirm('¿Estás seguro de eliminar este juego?')">
                                                    <i class="fas fa-trash mr-2"></i>
                                                </button>
                                            </form>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
        </div>
    </div>
@stop